...
__MSSHELL_WRAPPER_ static void _MS__private matrixSort(const sel_typ argc, char ** argv)
{
    ...
    else
    {
        ...
        for( ; ; )
        {
            ...
            printf2("A for User Matrix Inserting, B for Random Matrix Generation;\n");
            printf2("- %c to go Back..\n\n", EXIT_CHAR);

            PRINTL();

            do if((tmp = toupper(getch())) == EXIT_CHAR) return;
            while(tmp < 'A' && tmp > EXIT_CHAR);
...
__MSSHELL_WRAPPER_ static void _MS__private matrixEigenValues(const sel_typ argc, char ** argv)
{
    ...
    if((exit_state = _matrixEigenValues(matrix, eigenValues, eigenVectors, dim[ROWS])) == EIGVALUES_INFEVS_ERROR)
    	printErr(33, "No convergence after %hu iterations! Probably Complex EigenValues", MAX_EIGVALUES_ITERATIONS);
...
__MSSHELL_WRAPPER_ static void _MS__private matrixNorm(const sel_typ argc, char ** argv)
{
    ...
    else
    {
        ...
        printf2("- A for P-Norm, B for 1-Norm, C for Inf-Norm;\n");
    ...
    *(access(exprVars)->e_ANS) = nrm;
    ...
    printf2(OUTPUT_CONVERSION_FORMAT, nrm);
    printf2(".\n\n");
...
__MSSHELL_WRAPPER_ static void _MS__private matrixDet(const sel_typ argc, char ** argv)
{
    ...
    *(access(exprVars)->e_ANS) = dt;
    ...
    printf2(OUTPUT_CONVERSION_FORMAT, dt);
    printf2(".\n\n");
...
__MSSHELL_WRAPPER_ static void _MS__private matrixTrace(const sel_typ argc, char ** argv)
{
    ...
    *(access(exprVars)->e_ANS) = trc;
    ...
    printf2(OUTPUT_CONVERSION_FORMAT, trc);
    printf2(".\n\n");
...
__MSSHELL_WRAPPER_ static void _MS__private matrixSVD(const sel_typ argc, char ** argv)
{
    ...
    {
        printf2("- %llu: ", i);
        printf2(OUTPUT_CONVERSION_FORMAT, S[i]);
        PRINTN();
...
__MSSHELL_WRAPPER_ static void _MS__private matrixInv(const sel_typ argc, char ** argv)
{
    ...
    for(i=0; i<dim[ROWS]; ++i)
    {
        ...
        {
            printf2(OUTPUT_CONVERSION_FORMAT, *(matrix + (dim[ROWS]*i) + j));
            printf2(j == dimrows_per2-1 ? ";" : ",");
            PRINTSPACE();
        }
        PRINTN();
    }
...
__MSSHELL_WRAPPER_ static void _MS__private matrixAdd(const sel_typ argc, char ** argv)
{
    ...
    const bool complex_entries = access(algebra) != 0;
    const bool tensor_mode = __pmode__ == ALGOPS_TENSORSSUM;
    const sel_typ algebra_units = exp2(access(algebra));
    ...
    if(tensor_mode)
    {
        ...
        else
        {
            ...
            while(isNullVal((tmp = requires(NULL, NULL_CHAR, NULL_CHAR, PARSER_NOSETTINGS)))) 
            {
                CLEARBUFFER();
                if(isNullVal(tmp) && !access(exitHandle)) return;
    ...
    {
        if(argv_pos)
            printf2("Performing the Sum Matrices Component %s Operation.\n", k+1);
        ...
        {
            matrix1[k][i] = NULL;
            printf2("Enter IMAGINARY %s PART of the First Complex Matrix\n", suite_c.algebra_imaginary_units_names[access(algebra)][i]);
            printf2("by entering another [%hu x %hu] Matrix.\n\n", dim[ROWS], dim[COLUMNS]);
            if(argc > argv_pos)
            {
        ...
        if(complex_entries)
        {
            ...
            {
                printf2("\n+ %s*", suite_c.algebra_imaginary_units_names[access(algebra)][i]);
        ...
        else
        {
            ...
            printf2("Enter second [%hu x %hu] Matrix Elements.\n", dim[ROWS], dim[COLUMNS]);
            printf2("Elements Inserting Carriage will be automatically redirected on second scansion (for ROWS Scanning).\n\n");
        ...
        {
            matrix2[k][i] = NULL;
            printf2("Enter IMAGINARY %s PART of the Second Complex Matrix\n", suite_c.algebra_imaginary_units_names[access(algebra)][i]);
            printf2("by entering another [%hu x %hu] Matrix.\n\n", dim[ROWS], dim[COLUMNS]);
            if(argc > argv_pos)
            {
        ...
        printf2("\nMatrix Sum [%hu x %hu] between First and Second Matrix is:\n", dim[ROWS], dim[COLUMNS]);
        ...
        static void (* const matrixAddFuncs[_MAX_ALGEBRA])(ityp **, ityp **, ityp **, const register dim_typ [static MAX_DIMENSIONS]) =
   		{
    		_matrixAdd,
    		_matrixCAdd,
    		_matrixQAdd,
        ...
        matrixAddFuncs[access(algebra)](matrix1[k], matrix2[k], matrix_sum[k], dim);
...
__MSSHELL_WRAPPER_ static void _MS__private matrixMultiplication(const sel_typ argc, char ** argv)
{
    ...
    {
        matrix1[ii] = NULL;
        printf2("Enter IMAGINARY %s PART of the First Complex Matrix\n", suite_c.algebra_imaginary_units_names[access(algebra)][ii]);
        printf2("by entering another [%hu x %hu] Matrix.\n\n", dim[ROWS], dim[COLUMNS]);
        if(argc > argv_pos)
        {
    ...
    if(complex_entries)
    {
        ...
        {
            printf2("\n+ %s*", suite_c.algebra_imaginary_units_names[access(algebra)][ii]);
    ...
    if(assert_m)
    {
        ...
        else
        {
            ...
            while(isNullVal((tmp3 = requires(NULL, NULL_CHAR, NULL_CHAR, PARSER_NOSETTINGS))) || tmp3 != (tmp2 = (uint64_t)tmp3) || tmp2 < 1)
            {
                CLEARBUFFER();
                if(isNullVal(tmp3) && !access(exitHandle))
                {
                    #pragma omp parallel for num_threads(algebra_units)
					for(ii=0; ii<algebra_units; ++ii)
                        matrixFree(&matrix1[ii]);
    ...
    else
    {
        ...
        if(argc > argv_pos)
        {
            ...
            for(i=0; i<dim[COLUMNS2] && !(assert); ++i)
            {
                tmp = insertElement((*matrix2), (dim_typ2){0, i}, dim[COLUMNS2], false);
            ...
            for(i = !(assert); i<dim[COLUMNS]; ++i)
                for(j=start_col_index; j<dim[COLUMNS2]; ++j)
                {
                    tmp = insertElement((*matrix2), (dim_typ2){i, j}, dim[COLUMNS2], false);
    ...
    if(assert_m)
        printf2("First Matrix [%hu x %hu] raisen to the Power of: %llu is the [%hu x %hu] Matrix:", dim[ROWS], dim[ROWS], tmp2, dim[ROWS], dim[ROWS]);
    else
        printf2("Matrix Multiplication [%hu x %hu] between First and Second Matrix is:", dim[ROWS], dim[COLUMNS2]);

    PRINT2N();

    if(__pmode__ == ALGOPS_DOTPRODUCT)
        printf2("Whose unique element is the Dot Product between two inserted Vectors is:");
	...
	void (* matrixMulDispatcher )(ityp **, ityp **, ityp **, const register dim_typ [static MAX_MATRICES], const register sel_typ,
		...
		{
			if(dim[ROWS] >= MIN_STRASSEN_DIM && logan == (dim_typ)logan)
				matrixMulDispatcher = __call_STRASSENMM;
			else if(dim[ROWS] >= MIN_OSMM_DIM)
	...
	if(tmp2 != 1)
    {
        ...
        do
        {
			...
			matrixMulDispatcher(matrix1, matrix2, matrix_product, dim, algebra_units, matrixMulFuncs[access(algebra)], matrixAddFuncs[access(algebra)], matrixSubFuncs[access(algebra)]);
            if(assert_m)
                #pragma omp parallel for num_threads(algebra_units)
				for(ii=0; ii<algebra_units; ++ii)
...
__MSSHELL_WRAPPER_ static void _MS__private matrixKProduct(const sel_typ argc, char ** argv)
{
    ...
    if(assert_m)
    {
        ...
        else
        {
            ...
            while(isNullVal((tmp2 = requires(NULL, NULL_CHAR, NULL_CHAR, PARSER_NOSETTINGS))) || tmp2 != (tmp = (uint64_t)tmp2) || tmp < 1)
            {
                CLEARBUFFER();
                if(isNullVal(tmp2) && !access(exitHandle))
                {
                    #pragma omp parallel for num_threads(algebra_units)
					for(i=0; i<algebra_units; ++i)
                        matrixFree(&matrix1[i]);
    ...
    if(assert_m)
        printf2("First Matrix [%hu x %hu] raisen to the Kronecker Power of: %llu is the [%hu x %hu] Matrix:", dim[ROWS], dim[COLUMNS], tmp, dim3[ROWS], dim3[COLUMNS]);
    else
        printf2("Matrix Kronecker Product [%hu x %hu] between First and Second Matrix is:", dim3[ROWS], dim3[COLUMNS]);
    ...
    if(tmp != 1)
    {
  		...
  		do
        {
            ...
            matrixKProdFuncs[access(algebra)](matrix1, matrix2, matrix_product, dims);
...
__MSSHELL_WRAPPER_ static void _MS__private perScalarMultiplication(const sel_typ argc, char ** argv)
{
    ...
    {
        matrix[ii] = NULL;
        printf2("Enter IMAGINARY %s PART of the Complex Matrix\n", suite_c.algebra_imaginary_units_names[access(algebra)][ii]);
        printf2("by entering another [%hu x %hu] Matrix.\n\n", dim[ROWS], dim[COLUMNS]);
        if(argc > 1)
        {
    ...
    if(__pmode__ == ALGOPS_SCALARDIVISIONMATRIX)
    {
        ...
        printf2(" has been correctly divided by the Matrix:\n\n");
    }
    else
    {
        printf2("Matrix has been correctly multiplied by the Scalar ");
        printf2(OUTPUT_CONVERSION_FORMAT, scal);
        printf2(":\n\n");
    }
...
__MSSHELL_WRAPPER_ static void _MS__private illConditionChecking(const sel_typ argc, char ** argv)
{
    ...
    printf2(OUTPUT_CONVERSION_FORMAT, norms1*norms(matrix,dim[ROWS]));
    printf2(".\n\n");
