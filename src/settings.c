...
//
	...
	__MSSHELL_WRAPPER_ static void _MS__private __system changePrecision(const sel_typ argc, char ** argv)
	{
	    const fsel_typ old_precision = DEFAULT_PRECISION;
	    ityp tmp;
	    ...
	    if(argc)
	    {
	        if(PARSING_SYSTEM_ALLOWED)
	        {
	            ...
	            if((!parse(argv[0], &tmp)) || tmp != (DEFAULT_PRECISION = (fsel_typ)tmp) || DEFAULT_PRECISION < MIN_PRECISION || DEFAULT_PRECISION > MAX_PRECISION)
	            {
	            	DEFAULT_PRECISION = old_precision;
	        ...
	        else if((tmp = strtod(argv[0], NULL)) != (DEFAULT_PRECISION = (fsel_typ)tmp) || DEFAULT_PRECISION < MIN_PRECISION || DEFAULT_PRECISION > MAX_PRECISION)
	        {
	        	DEFAULT_PRECISION = old_precision;
	            printErr(33, "Invalid inserted Precision Value.\nMust be an integer between %hu and %hu", MIN_PRECISION, MAX_PRECISION);
	            printUsage(&change_settings[SETTINGS_CHANGEPRECISION]);
	    ...
	    else
	    {
	        ...
	        while((PARSING_SYSTEM_ALLOWED ? ((tmp = requires(NULL, NULL_CHAR, NULL_CHAR, PARSER_NOSETTINGS)) == NULL_VAL) :
	            (!scanf2(1, INPUT_CONVERSION_FORMAT, &tmp))) || tmp != (DEFAULT_PRECISION = (fsel_typ)tmp) || DEFAULT_PRECISION < MIN_PRECISION || DEFAULT_PRECISION > MAX_PRECISION)
	        {
	            CLEARBUFFER();
	...
	__MSSHELL_WRAPPER_ static void _MS__private __system changeStabilizerFactor(const sel_typ argc, char ** argv)
	{
	    const fsel_typ old_stabilizer_factor = DEFAULT_STABILIZER_FACTOR;
	    ityp tmp = 0.00;
	    ...
	    if(argc)
	    {
	        if(PARSING_SYSTEM_ALLOWED)
	        {
	            if((!parse(argv[0], &tmp)) || tmp != (DEFAULT_STABILIZER_FACTOR = (fsel_typ)tmp) || DEFAULT_STABILIZER_FACTOR < MIN_STABFACT || DEFAULT_STABILIZER_FACTOR > MAX_STABFACT)
	            {
	            	DEFAULT_STABILIZER_FACTOR = old_stabilizer_factor;
	        ...
	        else if((tmp = strtod(argv[0], NULL)) != (DEFAULT_STABILIZER_FACTOR = (fsel_typ)tmp) || DEFAULT_STABILIZER_FACTOR < MIN_STABFACT || DEFAULT_STABILIZER_FACTOR > MAX_STABFACT)
	        {
	        	DEFAULT_STABILIZER_FACTOR = old_stabilizer_factor;
	            printErr(33, "Invalid inserted Stabilizer Factor Value.\nMust be an integer between %hu and %hu", MIN_STABFACT, MAX_STABFACT);
	            printUsage(&change_settings[SETTINGS_CHANGESTABILIZERFACTOR]);
	    ...
	    else
	    {
	        ...
	        while((PARSING_SYSTEM_ALLOWED ? (isNullVal((tmp = requires(NULL, NULL_CHAR, NULL_CHAR, PARSER_NOSETTINGS)))) :
	            (!scanf2(1, INPUT_CONVERSION_FORMAT, &tmp))) || tmp != (DEFAULT_STABILIZER_FACTOR = (fsel_typ)tmp) || DEFAULT_STABILIZER_FACTOR < MIN_STABFACT || DEFAULT_STABILIZER_FACTOR > MAX_STABFACT)
	        {
	            CLEARBUFFER();
	            if(access(exitHandle) == EXITHANDLE_GETCMD) continue;
	            if(exitHandleCheck)
	    ...
	    if(getItemsListNo(ENVS) != STARTING_ENVSNO && access(exprVars)->e_ANS && isSett(BOOLS_SAVERESULTS))
	        *(access(exprVars)->e_ANS) = DEFAULT_STABILIZER_FACTOR;
	
	    sprint("Program Stabilizer Factor Value has been correctly changed from: %hu to: %hu.\n", old_stabilizer_factor, DEFAULT_STABILIZER_FACTOR);
	    return;
	}
#endif
	...
	__MSSHELL_WRAPPER_ static void _MS__private __system changeBlockSize(const sel_typ argc, char ** argv)
	{
	    const fsel_typ old_block_size= DEFAULT_BLOCKSIZE;
	    ityp tmp = 0.00;
	    ...
	    if(argc)
	    {
	        if(PARSING_SYSTEM_ALLOWED)
	        {
	            if((!parse(argv[0], &tmp)) || tmp != (DEFAULT_BLOCKSIZE = (fsel_typ)tmp) || DEFAULT_BLOCKSIZE < MIN_BLOCKSIZE)
	            {
	            	DEFAULT_BLOCKSIZE = old_block_size;
	        ...
	        else if((tmp = strtod(argv[0], NULL)) != (DEFAULT_BLOCKSIZE = (fsel_typ)tmp) || DEFAULT_BLOCKSIZE < MIN_BLOCKSIZE)
	        {
	        	DEFAULT_BLOCKSIZE = old_block_size;
	            printErr(33, "Invalid inserted Block Size Value.\nMust be an integer greater or equal than %hu", MIN_BLOCKSIZE);
	            printUsage(&change_settings[SETTINGS_CHANGEBLOCKSIZE]);
	    ...
	    else
	    {
	        ...
	        if(PARSING_SYSTEM_ALLOWED)
	            PRINTHOWTOBACKMESSAGE();
	        while((PARSING_SYSTEM_ALLOWED ? (isNullVal((tmp = requires(NULL, NULL_CHAR, NULL_CHAR, PARSER_NOSETTINGS)))) :
	            (!scanf2(1, INPUT_CONVERSION_FORMAT, &tmp))) || tmp != (DEFAULT_BLOCKSIZE = (fsel_typ)tmp) || DEFAULT_BLOCKSIZE < MIN_BLOCKSIZE)
	        {
	            CLEARBUFFER();
	            if(access(exitHandle) == EXITHANDLE_GETCMD) continue;
	            if(exitHandleCheck)
	...
	__MSSHELL_WRAPPER_ static void _MS__private __system changeMinOSMMDim(const sel_typ argc, char ** argv)
	{
	    const fsel_typ old_min_osmm_dim= DEFAULT_MINOSMMDIM;
	    ityp tmp = 0.00;
	    ...
	    if(argc)
	    {
	        if(PARSING_SYSTEM_ALLOWED)
	        {
	            if((!parse(argv[0], &tmp)) || tmp != (DEFAULT_MINOSMMDIM = (fsel_typ)tmp) || DEFAULT_MINOSMMDIM < MIN_OSMM_DIM)
	            {
	            	DEFAULT_MINOSMMDIM = old_min_osmm_dim;
	        ...
	        else if((tmp = strtod(argv[0], NULL)) != (DEFAULT_MINOSMMDIM = (fsel_typ)tmp) || DEFAULT_MINOSMMDIM < MIN_OSMM_DIM)
	        {
	        	DEFAULT_MINOSMMDIM = old_min_osmm_dim;
	            printErr(33, "Invalid inserted Min OSMM Dimension Value.\nMust be an integer greater or equal than %hu", MIN_OSMM_DIM);
	            printUsage(&change_settings[SETTINGS_CHANGEMINOSMMDIM]);
	    ...
	    else
	    {
	        ...
	        if(PARSING_SYSTEM_ALLOWED)
	            PRINTHOWTOBACKMESSAGE();
	        while((PARSING_SYSTEM_ALLOWED ? (isNullVal((tmp = requires(NULL, NULL_CHAR, NULL_CHAR, PARSER_NOSETTINGS)))) :
	            (!scanf2(1, INPUT_CONVERSION_FORMAT, &tmp))) || tmp != (DEFAULT_MINOSMMDIM = (fsel_typ)tmp) || DEFAULT_MINOSMMDIM < MIN_OSMM_DIM)
	        {
	            CLEARBUFFER();
	            if(access(exitHandle) == EXITHANDLE_GETCMD) continue;
	            if(exitHandleCheck)
	...
	__MSSHELL_WRAPPER_ static void _MS__private __system changeMinStrassenDim(const sel_typ argc, char ** argv)
	{
	    const fsel_typ old_min_strassen_dim= DEFAULT_MINSTRASSENDIM;
	    ityp tmp = 0.00;
	    ...
	    if(argc)
	    {
	        if(PARSING_SYSTEM_ALLOWED)
	        {
	            if((!parse(argv[0], &tmp)) || tmp != (DEFAULT_MINSTRASSENDIM = (fsel_typ)tmp) || DEFAULT_MINSTRASSENDIM < MIN_STRASSEN_DIM)
	            {
	            	DEFAULT_MINSTRASSENDIM = old_min_strassen_dim;
	        ...
	        else if((tmp = strtod(argv[0], NULL)) != (DEFAULT_MINSTRASSENDIM = (fsel_typ)tmp) || DEFAULT_MINSTRASSENDIM < MIN_STRASSEN_DIM)
	        {
	        	DEFAULT_MINSTRASSENDIM = old_min_strassen_dim;
	            printErr(33, "Invalid inserted Min Strassen Dimension Value.\nMust be an integer greater or equal than %hu", MIN_STRASSEN_DIM);
	            printUsage(&change_settings[SETTINGS_CHANGEMINSTRASSENDIM]);
	    ...
	    else
	    {
	        ...
	        if(PARSING_SYSTEM_ALLOWED)
	            PRINTHOWTOBACKMESSAGE();
	        while((PARSING_SYSTEM_ALLOWED ? (isNullVal((tmp = requires(NULL, NULL_CHAR, NULL_CHAR, PARSER_NOSETTINGS)))) :
	            (!scanf2(1, INPUT_CONVERSION_FORMAT, &tmp))) || tmp != (DEFAULT_MINSTRASSENDIM = (fsel_typ)tmp) || DEFAULT_MINSTRASSENDIM < MIN_STRASSEN_DIM)
	        {
	            CLEARBUFFER();
	            if(access(exitHandle) == EXITHANDLE_GETCMD) continue;
	            if(exitHandleCheck)
	    ...
	    if(getItemsListNo(ENVS) != STARTING_ENVSNO && access(exprVars)->e_ANS && isSett(BOOLS_SAVERESULTS))
	        *(access(exprVars)->e_ANS) = DEFAULT_MINSTRASSENDIM;
	
	    sprint("Program Min Strassen Dimension Value has been correctly changed from: %hu to: %hu.\n", old_min_strassen_dim, DEFAULT_MINSTRASSENDIM);
	    return;
	}
#endif
	...
	__MSSHELL_WRAPPER_ static void _MS__private __system changeOutlierConst(const sel_typ argc, char ** argv)
	{
	    const fsel_typ old_outlier_constant = DEFAULT_OUTLIER_CONSTANT;
		ityp tmp = 0.00; 
	    if(argc)
	    {
	    	ityp tmp = 0.00;
	        if(PARSING_SYSTEM_ALLOWED)
	        {
	            if((!parse(argv[0], &tmp)) || (DEFAULT_OUTLIER_CONSTANT=tmp) < MIN_OUTLIER_CONSTANT || DEFAULT_OUTLIER_CONSTANT > MAX_OUTLIER_CONSTANT)
	            {
	            	DEFAULT_OUTLIER_CONSTANT = old_outlier_constant;
	        ...
	        else if((DEFAULT_OUTLIER_CONSTANT = strtof(argv[0], NULL)) < MIN_STABFACT || DEFAULT_OUTLIER_CONSTANT > MAX_STABFACT)
	        {
	        	DEFAULT_OUTLIER_CONSTANT = old_outlier_constant;
	    ...
	    else
	    {
	        ...
	        if(PARSING_SYSTEM_ALLOWED)
	            PRINTHOWTOBACKMESSAGE();
	        while((PARSING_SYSTEM_ALLOWED ? (isNullVal((DEFAULT_OUTLIER_CONSTANT = (float)requires(NULL, NULL_CHAR, NULL_CHAR, PARSER_NOSETTINGS)))) :
	            (!scanf2(1, INPUT_CONVERSION_FORMAT, &DEFAULT_OUTLIER_CONSTANT))) || DEFAULT_OUTLIER_CONSTANT < MIN_OUTLIER_CONSTANT || DEFAULT_OUTLIER_CONSTANT > MAX_OUTLIER_CONSTANT)
	        {
	            CLEARBUFFER();
	            if(access(exitHandle) == EXITHANDLE_GETCMD) continue;
	            if(exitHandleCheck)
	    ...
	    if(getItemsListNo(ENVS) != STARTING_ENVSNO && access(exprVars)->e_ANS && isSett(BOOLS_SAVERESULTS))
	        *(access(exprVars)->e_ANS) = DEFAULT_OUTLIER_CONSTANT;
	
	    sprint("Program Outlier Constant has been correctly changed from: %.*f to: %.*f.\n", DEFAULT_PRECISION, old_outlier_constant, DEFAULT_PRECISION, DEFAULT_OUTLIER_CONSTANT);
	    return;
	}
#endif
	...
	__MSSHELL_WRAPPER_ static void _MS__private __system changeExitChar(const sel_typ argc, char ** argv)
	{
	    const char old_exit_char = EXIT_CHAR;
