...
case BCALC_SFACT:
    ...
    if(a)
    {
        ...
        c = DEFAULT_STABILIZER_FACTOR = a;
    }
    else
        c = DEFAULT_STABILIZER_FACTOR;
    break;
...
case BCALC_OUTLIERCONST:
    ...
    if(a < MIN_OUTLIER_CONSTANT || a > MAX_OUTLIER_CONSTANT)
        ...
        c = a;
        DEFAULT_OUTLIER_CONSTANT = a;
    }
    else
        c = DEFAULT_OUTLIER_CONSTANT;
    break;
...
case BCALC_VARIANZA:
{
    ...
    if(b)
    {
        ...
        if(!(accumulate))
            vector = malloc(sizeof(ityp)*(lazy_exec ? 1 : DEFAULT_STABILIZER_FACTOR));
        else if(lazy_exec)
            vector = realloc(vector, sizeof(ityp)*(accumulate+1));
        else if(!((accumulate+1) % DEFAULT_STABILIZER_FACTOR))
            vector = realloc(vector, sizeof(ityp)*(accumulate+DEFAULT_STABILIZER_FACTOR));
...
case BCALC_PERMUTATIONSREP:
{
    ...
    if(dcheck && sum != a)
    {
    	printErr(33, "You've inserted invalid multiplicities, because their sum isn't %.*f", DEFAULT_PRECISION, a);
		denominator = 1.00;
		sum = 0.00;
		return;
	}	
...
case BCALC_GENERATOREMATRICIRANDOM:
{
    ...
    if(dcheck && (a < 1 || b < 1 || a > MAX_RIGHE || b > MAX_COLONNE))
    {
